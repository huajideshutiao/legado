...
// Rhino 备用实现源码保留在 modules/rhino，但有意不 include：不配置 Gradle project，
// 因此不会参与任何编译、依赖解析或 APK 打包。未来重新启用时再显式 include 并接回依赖。
    ...
    configurations.all {
        resolutionStrategy {
            eachDependency {
                ...
                // components-resources 与 ui-text 的版本在 shared 各源集声明处按目标选择：
                // JVM/Android 使用官方 1.8.2，OHOS 使用具备 ohosArm64 变体的 1.8.2.99-alipay。
                // 这里不能全局回退，否则会把 fork 的传递依赖也降成不含 OHOS 变体的官方版本。
                if (requested.group == "org.jetbrains.compose.ui" && requested.name.startsWith("ui-tooling")) {
                    useVersion("1.8.2")
                    because("AntUI fork 未发布完整 JVM ui-tooling 变体, JVM/Android 预览使用官方基线版本")
                }
            }
            // force() 在依赖解析阶段生效, 优先级高于所有 eachDependency 规则
            // 原因: compose.material3/desktop DSL 访问器可能在 CMP fork 插件
