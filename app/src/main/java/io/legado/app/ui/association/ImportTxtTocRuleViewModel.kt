...
/**
 * 导入 TxtTocRule (目录规则) VM (Android 端, 组合委托)。
 *
 * # KMP 化重构说明
 *
 * 核心业务逻辑 (importSelect / importSource / importSourceAwait / importSourceUrl /
 * comparisonSource) 已下沉到 shared commonMain [ImportTxtTocRuleViewModelShared],
 * 用 `MutableStateFlow` 替代 `MutableLiveData` (LiveData 不可 KMP)。
 *
 * 本类采用**组合委托**模式持有 [shared] 实例, 不通过继承 [ImportTxtTocRuleViewModelShared]:
 * - 本类必须继承 [BaseViewModel] (AndroidViewModel 子类, 提供 `execute` / `context` /
 *   `viewModelScope`), Kotlin 单继承无法同时继承 [ImportTxtTocRuleViewModelShared];
...
class ImportTxtTocRuleViewModel(app: Application) : BaseViewModel(app) {
     ...
     */
    private val shared: ImportTxtTocRuleViewModelShared = ImportTxtTocRuleViewModelShared(
    ...
    /** 解析出的待导入 TxtTocRule 列表, 转发到 [shared.allSources] (同实例引用)。 */
    val allSources: ArrayList<TxtTocRule> get() = shared.allSources

    /** 已有 TxtTocRule, 转发到 [shared.checkSources] (同实例引用)。 */
    val checkSources: ArrayList<TxtTocRule?> get() = shared.checkSources
    ...
    /**
     * 导入选中的 TxtTocRule, 对应原 `importSelect(finally)`, 直接转发到 [shared.importSelect]。
     *
    ...
    /**
     * 导入 TxtTocRule, 对应原 `importSource(text)`。
     *
