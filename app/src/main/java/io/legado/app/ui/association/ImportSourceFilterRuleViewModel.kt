 ...
 * 已下沉到 shared commonMain [ImportSourceFilterRuleViewModelShared], 用
 * `MutableStateFlow` 替代 `MutableLiveData` (LiveData 不可 KMP)。
 *
 * 本类采用**组合委托**模式持有 [shared] 实例, 不通过继承 [ImportSourceFilterRuleViewModelShared]:
 * - 本类必须继承 [BaseViewModel] (AndroidViewModel 子类, 提供 `execute` / `context` /
 *   `viewModelScope`), Kotlin 单继承无法同时继承 [ImportSourceFilterRuleViewModelShared];
...
class ImportSourceFilterRuleViewModel(app: Application) : BaseViewModel(app) {
     ...
     */
    private val shared: ImportSourceFilterRuleViewModelShared =
        ImportSourceFilterRuleViewModelShared(scope = viewModelScope)
    ...
    /** 解析出的待导入过滤规则列表, 转发到 [shared.allRules] (同实例引用)。 */
    val allRules: ArrayList<SourceFilterRule> get() = shared.allRules

    /** 已有过滤规则, 转发到 [shared.checkRules] (同实例引用)。 */
    val checkRules: ArrayList<SourceFilterRule?> get() = shared.checkRules
    ...
    /**
     * 导入选中的过滤规则, 对应原 `importSelect(finally)`, 直接转发到 [shared.importSelect]。
     *
    ...
    /**
     * 导入过滤规则, 对应原 `import(text)` (注意方法名是 `import` 不是 `importSource`)。
     *
