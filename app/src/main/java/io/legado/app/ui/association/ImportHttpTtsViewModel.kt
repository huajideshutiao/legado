...
/**
 * 导入 HttpTTS 语音源 VM (Android 端, 组合委托)。
 *
 * # KMP 化重构说明
 *
 * 核心业务逻辑 (importSelect / importSource / importSourceAwait / importSourceUrl /
 * comparisonSource) 已下沉到 shared commonMain [ImportHttpTtsViewModelShared],
 * 用 `MutableStateFlow` 替代 `MutableLiveData` (LiveData 不可 KMP)。
 *
 * 本类采用**组合委托**模式持有 [shared] 实例, 不通过继承 [ImportHttpTtsViewModelShared]:
 * - 本类必须继承 [BaseViewModel] (AndroidViewModel 子类, 提供 `execute` / `context` /
 *   `viewModelScope`), Kotlin 单继承无法同时继承 [ImportHttpTtsViewModelShared];
...
class ImportHttpTtsViewModel(app: Application) : BaseViewModel(app) {
     ...
     */
    private val shared: ImportHttpTtsViewModelShared = ImportHttpTtsViewModelShared(
    ...
    /** 解析出的待导入 HttpTTS 列表, 转发到 [shared.allSources] (同实例引用)。 */
    val allSources: ArrayList<HttpTTS> get() = shared.allSources

    /** 已有 HttpTTS, 转发到 [shared.checkSources] (同实例引用)。 */
    val checkSources: ArrayList<HttpTTS?> get() = shared.checkSources
    ...
    /**
     * 导入选中的 HttpTTS, 对应原 `importSelect(finally)`, 直接转发到 [shared.importSelect]。
     *
    ...
    /**
     * 导入 HttpTTS, 对应原 `importSource(text)`。
     *
