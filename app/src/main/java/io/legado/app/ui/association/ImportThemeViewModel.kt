...
/**
 * 导入主题配置 VM (Android 端, 组合委托)。
 *
 * # KMP 化重构说明
 *
 * 核心业务逻辑 (importSelect / importSource / importSourceAwait / importSourceUrl /
 * comparisonSource) 已下沉到 shared commonMain [ImportThemeViewModelShared],
 * 用 `MutableStateFlow` 替代 `MutableLiveData` (LiveData 不可 KMP)。
 *
 * 本类采用**组合委托**模式持有 [shared] 实例, 不通过继承 [ImportThemeViewModelShared]:
 * - 本类必须继承 [BaseViewModel] (AndroidViewModel 子类, 提供 `execute` / `context` /
 *   `viewModelScope`), Kotlin 单继承无法同时继承 [ImportThemeViewModelShared];
 ...
 * - **ThemeConfig.Config → ThemeConfigData**: 原 app 端 allSources/checkSources 类型为
 *   `ArrayList<ThemeConfig.Config>`, 改为 `ArrayList<ThemeConfigData>` (KMP 版数据类,
 *   字段一一对应, commonMain 已下沉)。调用方 [ImportThemeDialog] 通过类型自动推导
 *   (`val local = viewModel.checkSources[it]`) 无需显式声明类型, 改动零扩散。
 *
 * # 调用方兼容
 *
 * [ImportThemeDialog] 调用方式保持不变:
 * - `viewModel.errorLiveData.observe(...)` / `viewModel.successLiveData.observe(...)`;
 * - `viewModel.allSources[it].themeName` (字段访问, ThemeConfigData 与 Config 同字段);
 * - `viewModel.allSources.size` / `viewModel.checkSources[it]` / `viewModel.selectStatus[it]`
 *   (getter 转发);
 * - `viewModel.isSelectAll` / `viewModel.selectCount` (getter 转发);
 * - `GSON.toJson(viewModel.allSources[index])` (Gson 反射, ThemeConfigData 是普通数据类,
 *   字段访问无差异; isBuiltin 字段在 app 端 Gson 会序列化, 但默认 false 且不影响业务,
 *   与原版 ThemeConfig.Config 的 `@Transient` 行为略有差异, 属显示细节);
...
class ImportThemeViewModel(app: Application) : BaseViewModel(app) {
     ...
     */
    private val shared: ImportThemeViewModelShared = ImportThemeViewModelShared(
    ...
    /** 解析出的待导入主题配置列表, 转发到 [shared.allSources] (同实例引用)。 */
    val allSources: ArrayList<ThemeConfigData> get() = shared.allSources

    /** 已有主题配置, 转发到 [shared.checkSources] (同实例引用)。 */
    val checkSources: ArrayList<ThemeConfigData?> get() = shared.checkSources
    ...
    /**
     * 导入选中的主题配置, 对应原 `importSelect(finally)`, 直接转发到 [shared.importSelect]。
     *
