 ...
 * - `R.drawable.image_loading_error` → 经 ImageErrorBytesProviders 注入 (资源单点在 :ui)
...
actual object ImageProvider {
    ...
    actual val errorBitmap: Bitmap by lazy {
        // 加载错误兜底图字节由宿主注入 (image_loading_error.png 单点持于 :ui
        // composeResources, 见 ImageErrorBytesProviders), 避免资源重复
        val bytes = ImageErrorBytesProviders.getOrNull()
            ?.invoke()
            ?: error("ImageErrorBytesProviders not registered")
