 ...
 * @JsApi 静态分派表语义锁定测试。
 *
 * [DispatchFixture] 经 kspAndroidTest 生成分派表; 每个场景用同一段 JS 分别在
...
class JsDispatchTableTest {
    ...
    fun testDispatcherRegisteredForFixture() {
        ...
        assertNotNull("kspAndroidTest 应已生成并注册 DispatchFixture 的分派表", d)
    ...
    fun testMethodBodyExceptionPropagates() {
        ...
        val js = "try { f.boom(); 'no-throw' } catch (e) { 'caught:' + String(e).indexOf('boom') >= 0 }"
