...
/**
 * TXT 本地书籍解析 (shared jvmAndAndroidMain 下沉版)。
 *
 * 原 app 端 `io.legado.app.model.fileBook.TextFile` 几乎纯 Kotlin:
 * - EncodingDetect / Utf8BomUtils 已下沉 commonMain
 * - MD5Utils / StringUtils / EmptyFileException / AppLog 已下沉 commonMain
 * - appDb 已下沉 (AppDbProviders)
 * - DefaultData.txtTocRules → DefaultDataShared.txtTocRules (commonMain)
 *
 * 放 jvmAndAndroidMain 原因: 依赖 java.io.InputStream / java.util.regex (JVM 标准库),
 * ios/ohos 端无本地 TXT 导入需求, 不下沉到 commonMain。
 *
 * 依赖说明:
 * - BaseFileBook: 已下沉 shared/commonMain (getBookInputStream/getLastModified 委托 FileBookProviders)
 * - 原 FileBook.getBookInputStream(book) → TextFile.getBookInputStream(book)
 *   (TextFile companion 继承 BaseFileBook 默认方法, 委托 FileBookProviders, 与原语义一致;
 *   FileBook object 仍在 app 端, 由 FileBookAccessorImpl 包装注册)
 * - isLocalModified: 已下沉 shared/src/jvmAndAndroidMain/.../BookExtensionsLocalModified.jvmAndAndroid.kt
 *   (用 FileBookProviders.get().getLastModified 替代原 FileBook.getLastModified)
 *
 * 跨模块同包名同签名扩展自动合并, app 端消费方 import 零改动。
 */
class TextFile(private var book: Book) {
    ...
    fun getChapterList(): ArrayList<BookChapter> {
        ...
        if (book.charset == null || book.tocUrl.isBlank() || modified) {
            FileBookProviders.get().getBookInputStream(book).use { bis ->
    ...
    private fun getTocRules(): List<TxtTocRule> {
        // Room KMP: DAO 属性已改为 suspend 方法，getTocRules 不能 suspend (调用链通向 override 非 suspend 的 getChapterList)
        var rules = runBlocking { AppDbProviders.get().txtTocRuleDao.enabled() }
        if (runBlocking { AppDbProviders.get().txtTocRuleDao.count() } == 0) {
            rules = DefaultDataShared.txtTocRules.apply {
                runBlocking { AppDbProviders.get().txtTocRuleDao.insert(*this@apply.toTypedArray()) }
            }.filter {
                it.enable
            }
        }
