 ...
 *
 * - 原逻辑零变化: OkHttp + WebDavApi (PROPFIND/MKCOL/PUT/DELETE/GET)
 * - `DEFAULT_CONTENT_TYPE` 提升为 commonMain top-level `const val`, actual 直接引用
 * - `upload(file: File, ...)` 重载不在 expect 中 (File 非 KMP 类型), 作为 actual 额外成员保留
 * - 各 actual 方法移除默认参数值 (KMP 规则: 默认值只在 expect 中声明)
 */
...
actual open class WebDav actual constructor(
    ...
    val webDavClient by lazy {
        ...
        OkHttpClientProviders.get().okHttpClient.newBuilder().run {
    ...
    private fun parseBody(s: String): List<WebDavFile> {
        ...
        for (element in elements) {
            ...
            val hrefDecode = UrlPathDecoder.decode(href, Charsets.UTF_8)
            ...
            try {
                ...
                val displayName = element
                    ...
                    ?.let { UrlPathDecoder.decode(it, Charsets.UTF_8) } ?: fileName
    ...
    /**
     * 上传文件 (jvmAndAndroid 额外重载, 不在 commonMain expect 中: File 非 KMP 类型)
     */
    @Throws(WebDavException::class)
