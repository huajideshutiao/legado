 ...
 * ([SourceDecodeCacheStrategy]) 的 url 进表, 真正发起网络请求前拦截。
 *
...
/** 上限防进程级无界增长 (淘汰最早记录, 容量内语义与原无界表一致)。 */
private const val FAIL_URLS_CAPACITY = 512
private val failUrls = LinkedHashSet<String>()

internal fun isFailUrl(url: String): Boolean = synchronized(failUrlsLock) { url in failUrls }

internal fun markFailUrl(url: String) {
    synchronized(failUrlsLock) {
        if (!failUrls.add(url)) return
        if (failUrls.size > FAIL_URLS_CAPACITY) failUrls.remove(failUrls.first())
    }
}
...
internal class ImageGuardNetworkClient(
    ...
    override suspend fun <T> executeRequest(
        ...
        return try {
            ...
            // [SourceDecodeCacheStrategy].write 落盘, 死链不污染磁盘缓存);
