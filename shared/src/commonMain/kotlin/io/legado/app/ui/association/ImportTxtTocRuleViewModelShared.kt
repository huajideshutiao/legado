...
/**
 * 导入 TxtTocRule (目录规则) VM 共享核心 (KMP 版, commonMain)。
 *
 * # 背景
 *
 * 对照 app 端原 `ImportTxtTocRuleViewModel(app: Application) : BaseViewModel(app)`:
 ...
 *   AppDbAccessorImpl, 已暴露 txtTocRuleDao DAO)。
 ...
 *
 * - importSelect(finally): 遍历 selectStatus 选中的 TxtTocRule, 批量 insert 到 txtTocRuleDao;
 ...
 * - comparisonSource() [private]: 遍历 allSources 调 txtTocRuleDao.get(id), 本地不存在
 *   或与本地不同则选中, 推送 successState。
 *
 * # Android 专属依赖替换
 *
 ...
 *   下沉后 app 端 `ImportTxtTocRuleViewModel.importSource(text)` 先判断 isUri, 若是 Uri
 *   则先读取文本再传入本类 [importSource]; 否则直接转发到本类 [importSource]。
 *   URL/JSON 解析逻辑全部下沉到本类。
 * - **appDb.txtTocRuleDao**: 改为 [AppDbProviders.get].txtTocRuleDao (宿主注册)。
 ...
 * - app 端 `ImportTxtTocRuleViewModel(application)` `extends BaseViewModel(application)`,
