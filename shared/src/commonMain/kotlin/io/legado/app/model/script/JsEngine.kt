 ...
 *
 * - **引擎选择**：硬编码 QUICKJS（见 [JsEngines.type]），rhino 已弃用（模块已删, 见 git 历史 modules/rhino）。切换入口已撤。
 ...
 * - **引擎绑定留 app**：`QuickJsJsEngine` 及其 `SharedJsScopeProvider`
 *   强依赖 `modules/quickjs`（JNI），不下沉 shared；
 ...
 */
enum class JsEngineType { QUICKJS }
