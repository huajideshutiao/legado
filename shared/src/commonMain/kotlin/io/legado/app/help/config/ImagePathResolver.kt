...
/** 本地提取与遗留封面相对引用首段 (`oldCovers/<md5>.jpg`, 见 [resolveImagePath] 解析规则)。 */
const val OLD_COVERS_REF_SEGMENT = "oldCovers"
 ...
 *
 * # 约定
 * 设置点 (主题背景图/启动图/阅读背景图) 存**图集相对引用**: 裸文件名
 * （如 `<字节数>.webp`，不含目录前缀，文件落 `{externalFiles|files}/customImg`
 * 图集目录；目录前缀不进入引用，目录整体随备份打包），读取端经本函数解析为绝对路径 ——
 * 跨机/跨端恢复备份时相对引用自动有效。
 *
 * 解析规则:
 * - **本地提取与遗留封面相对引用** (`oldCovers/<name>`, [OLD_COVERS_REF_SEGMENT], 兼容历史 `coverCache/<name>`):
 *   物理落盘平台 `coversDir`, 不入 customImg 图集
 *   (那是自定义封面 `covers/<字节数>.<ext>` 的保留段, 两者同为 Book 封面字段但目录语义不同);
 *   平台未注册 `coversDir` 时原样返回 (加载失败走占位)
...
fun resolveImagePath(ref: String?): String? {
    ...
    // 本地提取与遗留封面引用: 对准平台 covers 物理目录, 不经 customImg 图集 (兼容历史 coverCache 前缀)
    if (rawSegments[0] == OLD_COVERS_REF_SEGMENT || rawSegments[0] == LEGACY_COVER_CACHE_REF_SEGMENT) {
