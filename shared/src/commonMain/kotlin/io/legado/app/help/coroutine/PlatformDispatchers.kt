...
/**
 * Coroutine 内部错误打印。原 app 侧 `utils.printOnDebug` 仅 DEBUG 打栈；
 * 下沉后为避免反向依赖 app, 收为同包 expect, 各平台自定「是否 debug」。
 *
 * P0-0b: 改 public 让 app 端 LogUtils.kt 的 printOnDebug 可转发到本 expect/actual,
 * 消除 BuildConfig.DEBUG 检查的重复逻辑。app 端调用方 import 零改动。
 */
expect fun Throwable.printOnDebug()
