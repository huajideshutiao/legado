...
/**
 * 鸿蒙端 [AppLogHost] 实现: 落盘走 [NativeAppLogStore] + toast 走 napi 桥。
 *
 * # 与桌面端 registerDesktopAppLogHost 对照
 * - [write] → 开启"记录日志" (PreferKey.recordLog) 时经 [NativeAppLogStore] 追加到
 *   `{filesDir}/logs/appLog-<epochMillis>.txt` (落盘实现已上提 nativeMain, iOS 共用)
 ...
 * - [timeZoneOffsetMillis] → nativeMain 的 POSIX localtime_r 换算 (与 iOS 端同一份,
 *   日志 UI 按本地时间显示)
 *
 * # 崩溃日志
 * 日志文件即崩溃日志来源: [NativeCrashLogs] 列出 `{filesDir}/logs` 下的 appLog-*.txt 作为
