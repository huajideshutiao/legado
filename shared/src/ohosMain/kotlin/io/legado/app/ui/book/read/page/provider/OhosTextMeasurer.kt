...
/**
 * 鸿蒙端 [TextMeasurer]: 走 Skia 真实字形度量 (与 desktop SkiaTextMeasurer / iOS IosTextMeasurer
 * 同源同口径), 取代等宽近似 [SimpleTextMeasurer]。
 *
 ...
 *
 * # 与绘制侧同源
 * 绘制侧 (ReaderDrawStyle 的 Compose FontFamily.Default) 在鸿蒙映射到系统字体
 * (HarmonyOS Sans); 这里按同一份别名表向 [FontMgr.default] 取, 用户自定义字体
 * (ReadBookConfig.textFont) 走 makeFromFile 读同一文件。
 *
 * 注册: [registerOhosTextMeasurer] (registerOhosProviders 中), 未注册仍回退 SimpleTextMeasurer。
 */
...
/** 注册鸿蒙真实字形度量器 (registerOhosProviders 中调用; 未注册回退 SimpleTextMeasurer 等宽近似)。 */
fun registerOhosTextMeasurer() {
    TextMeasurerProviders.register { textSizePx, letterSpacingPx, fontPath ->
        OhosTextMeasurer(
            textSizePx = textSizePx,
            letterSpacingPx = letterSpacingPx,
            typeface = OhosTextMeasurer.readerTypeface(fontPath),
        )
    }
}
