 ...
 *   鸿蒙端 onUpPath 无法加载远程书籍列表, items 永远为空 (显示空态)
 *   (注: nativeMain 中已有 `lib/webdav/WebDav.native.kt`, 但 ViewModel 层桥接未接入)
 ...
 *   onShowWebDavHelp / onShowLogDialog 改用 [AlertDialog] 显示未实现提示 (与 desktop/iOS 一致)
 ...
 */
@Composable
fun OhosRemoteBookScreen(onBack: () -> Unit) {
    OhosRemoteBookContent(onBack = onBack)
}

/**
 * 鸿蒙端远程书籍条目模型 (替代 app 端 RemoteBook; 与 iOS `IosRemoteBook` 字段一致)。
 *
 * app 端 [io.legado.app.model.remote.RemoteBook] 基于 `WebDavFile` (依赖 WebDav 客户端),
 * WebDav 客户端未下沉到 commonMain, 鸿蒙端用本类替代:
 ...
 *
 * 鸿蒙端因 WebDav 客户端未下沉, 实际不会构造本类实例 (items 永远为空), 字段保留供
 * 后续接入 WebDav 后直接复用。
 */
private data class OhosRemoteBook(
    val filename: String,
    val path: String,
...
@Composable
private fun OhosRemoteBookContent(onBack: () -> Unit) {
    ...
    // 鸿蒙端直接复用 shared/sharedUiMain 下沉的 ServersDialog / ServerConfigDialog Composable。
    ...
    val webDavClientNotAvailableLogText = rememberString("ohos_webdav_client_not_available_log")
    ...
    // 列表条目 (鸿蒙端 WebDav 客户端未下沉, 永远为空)
    var items by remember { mutableStateOf<List<OhosRemoteBook>>(emptyList()) }
    // 选中集
    var selected by remember { mutableStateOf<Set<OhosRemoteBook>>(emptySet()) }
    // 上架标记原地变更后 +1 强制列表重组 (对照 RemoteBookActivity.refreshTick)
    var refreshTick by remember { mutableIntStateOf(0) }
    // 子目录栈 (面包屑路径用, 空表示在根目录)
    val dirList = remember { mutableListOf<OhosRemoteBook>() }
    // 面包屑路径 (null 时不显示)
    ...
    // 排序升降序 (对照 RemoteBookViewModel.sortAscending, 鸿蒙端虽无数据但保留状态)
    var sortAscending by remember { mutableStateOf(true) }

    fun isCheckable(item: OhosRemoteBook): Boolean =
    ...
    fun upPath() {
        ...
        //  鸿蒙端暂不实现远程书籍加载, items 保持空, 显示空态
        //  (nativeMain 已有 lib/webdav/WebDav.native.kt, 但 ViewModel 桥接未接入)
        var p = "books/"
    ...
    fun sortCheck(sortKey: RemoteBookSort) {
        ...
        // 鸿蒙端 items 为空, 排序切换无实际效果, 仅更新状态
        upPath()
    }

    // ---- UiActions 实现 (remember 持有稳定实例) ----
    val actions = remember {
        object : RemoteBookUiActions<OhosRemoteBook> {
            override fun onBack() {
            ...
            override fun onUpSearchKey(key: String) {
                searchKey = key
                // TODO: app 端调 viewModel.updateCallBackFlow(newText) 重过滤, 鸿蒙端无数据 no-op
            }
            ...
            override fun onItemClick(item: OhosRemoteBook) {
                when {
                    item.isUpDir -> {
            ...
            override fun onItemLongClick(item: OhosRemoteBook) {
    ...
    // 服务器列表 + 选中态 + 增删改全部由 shared ServersDialog 内部管理, 鸿蒙端仅注入数据与回调。
    if (showServersDialog) {
        ServersDialog(
            ...
            onSelectDefault = {
                ...
                // (鸿蒙 AppConfigAccessor 为只读, 直接写 PreferenceProvider, 与 desktop/iOS 一致)
