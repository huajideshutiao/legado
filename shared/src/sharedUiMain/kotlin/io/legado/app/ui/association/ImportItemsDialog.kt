...
fun ImportItemsDialog(
    ...
    version
    val stateNew = rememberString("import_state_new")
    val stateUpdate = rememberString("import_state_update")
    val stateExisting = rememberString("import_state_existing")
...
/**
 * 书源勾选导入对话框: 通用对话框 + 标题栏溢出菜单 (选中新增源/选中更新源)。
 * app 端另有 分组/保留名称等 菜单项, 依赖可写 AppConfig 与 alert DSL, 暂未下沉。
 */
@Composable
            ...
            Box {
                ...
                AppDropdownMenu(expanded = showMenu, onDismissRequest = { showMenu = false }) {
                    DropdownMenuItem(onClick = {
                        ...
                        adapter.selectNew()
                        invalidate()
                    }) { Text(rememberString("select_new_source")) }
                    DropdownMenuItem(onClick = {
                        showMenu = false
                        adapter.selectUpdate()
                        invalidate()
                    }) { Text(rememberString("select_update_source")) }
                }
            }
        },
    )
