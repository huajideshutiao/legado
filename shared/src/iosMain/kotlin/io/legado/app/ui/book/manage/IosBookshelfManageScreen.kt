 ...
 *   复刻 app 端边缘拖选批量勾选语义
 * - **封面槽**: 注入 [IosInfoCover] (UIImage + Skia ImageBitmap, 不引入 Glide)
 *
 ...
 * - **批量改源**: [BookshelfManagePlatform] 未在 iOS 端注册, onSelectActions "批量改源" no-op
 * - **打开书籍详情**: 由 [onOpenBook] 回调注入, 切到 BOOK_INFO 路由
 *
 * @param onBack 返回回调 (切回 BOOKSHELF 路由, 由 IosNavHost 注入)
 * @param onOpenBook 点击封面打开书籍详情回调 (携带 Book, 切到 BOOK_INFO 路由)
 */
@Composable
fun IosBookshelfManageScreen(
    onBack: () -> Unit,
            ...
            onSelectActions = {
                listOf(
                    ...
                    SelectAction(changeSourceBatchLabel) {
                        // TODO: BookshelfManagePlatform 未在 iOS 端注册, 批量改源暂不实现
                    },
                    SelectAction(clearCacheLabel) {
                        // TODO: BookHelp.clearCache 未在 iOS 端注册, 清缓存暂不实现
                    },
                )
            },
    ...
    io.legado.app.ui.book.manage.BookshelfManageScreen(
        ...
        listState,
        dragListModifier,
        { book -> IosInfoCover(book = book, modifier = Modifier) },
    )
