...
/**
 * iOS 端书源编辑 Screen 入口 (包装 shared/sharedUiMain 的 [io.legado.app.ui.book.source.edit.BookSourceEditScreen])。
 *
 * 对照 desktop `BookSourceEditScreen.kt` 的包装模式, iOS 端在 [IosBookSourceScreen] 的
 * onEdit 回调中用 Dialog 全屏展示本入口。Provider 由 MainViewController 全局注入,
 * 无需在此重复包装。
 *
 * 业务逻辑 (upSourceView / getSource / saveSource) 与 desktop / app 端一致,
 * 仅做 iOS 平台适配 (NSDate 替代 System.currentTimeMillis, openURL 替代 browseUrl)。
 *
 * 简化项 (与 desktop 对比):
 * - 复制/粘贴/分享: iOS 端无 AWT 剪贴板, 暂留 no-op + TODO
 ...
 * @param onDebugSource 调试回调 (切到调试 Dialog)
 */
@Composable
fun IosBookSourceEditScreen(
    sourceUrl: String,
    onBack: () -> Unit,
    onSaved: (String) -> Unit,
...
private fun BookSourceEditContent(
    ...
    // 保存书源 (对照 app 端 ViewModel.save, iOS 用 NSDate 替代 System.currentTimeMillis)
    fun saveSource() {
        ...
        scope.launch {
            try {
                ...
                if (!source.equal(oldSource)) {
                    source.lastUpdateTime = (NSDate().timeIntervalSince1970() * 1000).toLong()
    ...
    val callbacks = remember(onBackCallback, onSavedCallback, onDebugSource) {
        BookSourceEditCallbacks(
            ...
            onSearch = {
                // TODO: 依赖搜索路由, iOS 端暂未接入
            },
            onClearCookie = {
                // TODO: 依赖 CookieStore, iOS 端未下沉 Cookie 管理
            },
            onCopySource = {
                // TODO: iOS 端无 AWT 剪贴板, 后续接入 UIPasteboard
            },
            onPasteSource = {
                // TODO: iOS 端无 AWT 剪贴板, 后续接入 UIPasteboard
            },
            onAutoIndent = {
                // TODO: 依赖 CodeView (Android 专属), iOS 端 OutlinedTextField 无对应能力
            },
            onSetSourceVariable = {
                showVariableDialog = true
            },
            onShareSourceStr = {
                // TODO: iOS 端无 AWT 剪贴板, 后续接入 UIActivityViewController
            },
            onHelp = { help(it) },
            hasLogin = { hasLogin() },
