...
/**
 * iOS 端 TXT 目录规则管理 Screen 入口 (包装 shared/sharedUiMain 的 [TxtTocRuleScreen])。
 *
 * # 职责
 *
 * 对照 app 端 [TxtTocRuleActivity] 的薄壳模式, iOS 端在 [io.legado.app.ui.IosNavHost]
 * 的 TXT_TOC_RULE 路由分支调用本入口。
 *
 ...
 *   HandleFileContract.EXPORT "exportTxtTocRule.json"); 保存失败/取消降级复制到剪贴板;
        ...
        actions = object : TxtTocRuleUiActions {
            ...
            override fun onExportSelection() {
                ...
                scope.launch {
                    ...
                    val saved = exportFile("exportTxtTocRule.json", json.encodeToByteArray())
                    if (saved) {
