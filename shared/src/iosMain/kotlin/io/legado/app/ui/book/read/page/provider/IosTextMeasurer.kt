...
/**
 * iOS 端 [TextMeasurer]: 走 Skia 真实字形度量 (与 desktop SkiaTextMeasurer 同源同口径),
 ...
 *
 * # 与绘制侧同源
 * 绘制侧 (ReaderDrawStyle 的 Compose FontFamily.Default) 在 iOS 映射到系统字体;
 * 这里按同一份别名表 (.AppleSystemUIFont / PingFang SC 等) 向 [FontMgr.default] 取,
 * 用户自定义字体 (ReadBookConfig.textFont) 走 makeFromFile 读同一文件。
 *
 * 注册: [registerIosTextMeasurer] (registerIosProviders 中), 未注册仍回退 SimpleTextMeasurer。
 */
class IosTextMeasurer(
    ...
    companion object {
        ...
        /** 回退匹配的 locale 标签 (iOS 中文正文场景) — 与绘制侧 SkParagraph 的 locale 语义近似。 */
...
/** 注册 iOS 真实字形度量器 (registerIosProviders 中调用; 未注册回退 SimpleTextMeasurer 等宽近似)。 */
fun registerIosTextMeasurer() {
    TextMeasurerProviders.register { textSizePx, letterSpacingPx, fontPath ->
        IosTextMeasurer(
            textSizePx = textSizePx,
            letterSpacingPx = letterSpacingPx,
            typeface = IosTextMeasurer.readerTypeface(fontPath),
        )
    }
}
