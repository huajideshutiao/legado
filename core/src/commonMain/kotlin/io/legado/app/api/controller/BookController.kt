...
object BookController {
    ...
    * 分组号及名称
     ...
     * 默认封面位图经 [ImageControllerProvider.defaultCoverBytes] 取内置兜底封面字节。
     */
    suspend fun getCover(parameters: Map<String, List<String>>): ReturnData {
        ...
        if (bytes != null) {
            ...
            // 试 defaultCoverBitmap/内置图后 setData 成功); 默认封面字节经 provider 注入
            // (composeResources 归 :ui, 见 ImageControllerProvider.defaultCoverBytes)
            val defaultBytes = try {
                ImageControllerProviders.get().defaultCoverBytes()
            } catch (e: Exception) {
                null
            }
...
interface ImageControllerProvider {
     ...
     * 默认封面位图字节流 (内置兜底封面)。
     *
     * 原 BookController.getCover 直接经 composeResources 的 Res.readBytes 取内置封面;
     * 切分后 composeResources 归 :ui, :data 不引 compose, 故改为经本 provider 注入,
     * 由持有资源的宿主 (app/desktop) 实现, 未提供时返回 null (调用方 setErrorMsg)。
     */
    fun defaultCoverBytes(): ByteArray? = null
}
