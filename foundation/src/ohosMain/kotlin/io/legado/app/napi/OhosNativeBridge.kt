...
object OhosNativeBridge {
    ...
    /**
     * 平台事件分发通道 (由上层 :ui 的 [io.legado.app.napi.OhosPlatformEventChannel]
     * 实现并注册)。本模块只持转发入口, 避免 foundation 反向依赖 ui 层。
     */
    fun interface PlatformEventChannel {
        fun onEvent(eventJson: String)
    }

    @Volatile
    private var platformEventChannel: PlatformEventChannel? = null

    /** 注入平台事件分发通道 (由 :ui 启动注册时调用)。 */
    fun registerPlatformEventChannel(channel: PlatformEventChannel) {
